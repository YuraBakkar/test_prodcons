...
Якщо Producer завершується або аварійно падає, Consumer розпізнає видалення чи
заміну shared-memory object за його системною ідентичністю (`st_dev` і
`st_ino`), від'єднує старий mapping і знову переходить до очікування. Після
перезапуску Producer прийом автоматично відновлюється без перезапуску Consumer.
...
Producer аналогічно утримує lock у `/tmp/test_prodcons_producer_<uid>.lock`.
Другий Producer не може працювати одночасно з першим. Після аварійного
завершення lock автоматично звільняється ядром, а новий Producer видаляє
залишену shared memory та створює чисту область.
